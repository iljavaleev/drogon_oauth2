...
State Client::getState(const DbClientPtr &clientPtr) const {
    static const std::string sql = "select * from state where client_id = $1";
    Result r(nullptr);
    {
        auto binder = *clientPtr << sql;
        binder << *clientId_ << Mode::Blocking >>
...
void Client::getState(const DbClientPtr &clientPtr,
